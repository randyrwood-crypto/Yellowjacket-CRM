...
The app starts with no logins at all — you create the first admin account
once, from Render's built-in terminal (called the **Shell**):

1. Open your web service in the Render dashboard.
2. Click the **Shell** tab (in the left-hand menu, or the top tab bar
   depending on your plan — it's a terminal running inside your live app).
3. Run:
   ```bash
   npm run seed-admin -- "Randy Wood" "choose-a-real-password"
   ```
   Use your real name and a password only you know — this becomes your
   admin login. You can run this command again later (with the same name)
   to reset that password if you forget it.

That's it. Open the site URL, click the **Admin** tab, sign in with the
name and password you just set, and go to the **Team** page to add your
salesmen (Jerrett, etc.) — each with their own temporary password, which
they should change isn't required but you can add a "change password"
step later if you want one.
